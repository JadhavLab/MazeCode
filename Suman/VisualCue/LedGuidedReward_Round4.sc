...
% Output Ports
int leftLED = 0 % at port 1 but setting to 0 to inactivate
int rightLED = 0 % at port 2 but setting to 0 to inactivate
int leftRewardWellPump = 3
int rightRewardWellPump = 4
int odorWellLED = 0 % at port 5 but setting to 0 to inactivate
int leftIndicator = 6
...
% This function determines whether a nose poke is in the correct reward well or not. If it is correct it dispenses reward
function 2
...
% This function takes care of the odor well and chooses using a random number generating function the next reward well that will be activated
function 1
  if (startTrial == 1) do
