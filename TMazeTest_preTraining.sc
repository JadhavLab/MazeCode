% PROGRAM NAME: 	T MAZE PRETRAINING
% AUTHOR: 			MD
% DESCRIPTION:
% Prepare animals for full T maze test by only performing the forced-choice sampling phase. 

%CONSTANTS

int deliverPeriod= 500   % how long to deliver the reward
int delayPeriod = 0  % 3 sec delay, can be changed
...
%                end
%            end
                 if ( delayPeriod != 0 ) do in 500  % delay of triggering home port door
                      portout[homedoor] = 1
                      do in delayPeriod
                                 portout[homedoor] = 0
                                 disp('Trial start')
