...
callback portin[3] up
	...
	if (lastWell !=1 && wellStatA==1) 	do				% Check if previous well = center
	 ...
	 if (wellStatA==1 && lastWell1 !=1) 	do				% Check if previous well = center
			disp('Rewarding - position A1 other ')
			rewardWell=rewardPump1 	% dispense reward from here	
			trigger(1)					% trigger reward
	end
	lastWell1=1	
...
callback portin[2] up
	...
	if (wellStatB==1 && lastWell1 !=2 ) 	do				% Check if previous well = center
			disp('Rewarding position B2 other')
			rewardWell=rewardPump2 	% dispense reward from here
			lastWell=2		
			trigger(1)					% trigger reward
	end
...
callback portin[8] up
	...
	wellStat3=1
	if (lastWell !=3 && wellStatC==1) 	do				% Check if previous well = center
			disp('Rewarding position C3 ')
			rewardWell=rewardPump3 	% dispense reward from here
			lastWell=3	
			trigger(1)					% trigger reward
	end
	if (wellStatC==1 && lastWell1 !=3) 	do				% Check if previous well = center
			disp('Rewarding position C3other ')
			rewardWell=rewardPump3 	% dispense reward from here
			lastWell=3	
			trigger(1)					% trigger reward
	end
...
callback portin[1] up
	...
	if (wellStat1==1 && lastWell2 !=1) do					% Check if previous well = center
			disp('Rewarding position A1other ')
			rewardWell=rewardPump1 	% dispense reward from here
			lastWell=1	
			trigger(1)					% trigger reward
	end
...
callback portin[9] up
	...
	if (wellStat2==1&& lastWell2 !=2 ) do				% Check if previous well = center
			disp('Rewarding position B2 other')
			rewardWell=rewardPump2 	% dispense reward from here
			lastWell=2		
			trigger(1)					% trigger reward
...
callback portin[11] up
	...
	wellStatC=1
	if (lastWell !=3 && wellStat3==1) 	do				% Check if previous well = center
			disp('Rewarding position C3 ')
			rewardWell=rewardPump3	% dispense reward from here
			lastWell=3		
			trigger(1)					% trigger reward
	end
	if (wellStat3==1&& lastWell2 !=3 ) 	do				% Check if previous well = center
			disp('Rewarding position C3other ')
			rewardWell=rewardPump3	% dispense reward from here
			lastWell=3	
			trigger(1)					% trigger reward
