...
function 2
	goalwell=random(2)+1
	while (goalwell==lastWell) do every 10
		goalwell=random(2)+1
	end
end
...
callback portin[3] up
	...
	wellStat1=1
	if (lastWell !=1 && wellStatA==1 && goalwell==1) 	do				% Check if previous well = center
			...
			disp(goalwell)					% trigger new reward well
...
callback portin[2] up
	...
	wellStat2=1
	if (lastWell !=2 && wellStatB==1 && goalwell==2) 	do				% Check if previous well = center
...
callback portin[8] up
	...
	wellStat3=1
	if (lastWell !=3 && wellStatC==1 && goalwell==3) 	do				% Check if previous well = center
...
callback portin[4] up
	...
	wellStatA=1
	if (lastWell !=1 && wellStat1==1 && goalwell==1) do					% Check if previous well = center
			disp('Rewarding position A ')
			rewardWell=rewardPump1 	% dispense reward from here
			lastWell=1		
			trigger(1)					% trigger reward
			trigger(2)
...
callback portin[9] up
	...
	wellStatB=1
	if (lastWell !=2 && wellStat2==1&& goalwell==2) 	do				% Check if previous well = center
			disp('Rewarding position B2 ')
			rewardWell=rewardPump2 	% dispense reward from here
			lastWell=2		
			trigger(1)					% trigger reward
...
callback portin[11] up
	...
	wellStatC=1
	if (lastWell !=3 && wellStat3==1&& goalwell==3) 	do				% Check if previous well = center
			disp('Rewarding position C3 ')
			rewardWell=rewardPump3	% dispense reward from here
			lastWell=3		
			trigger(1)					% trigger reward
